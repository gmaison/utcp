...
`.repeat(3);
    ...
    // Trim to 50k if needed
    const TRIM_LENGTH = 50000;
    const trimmedText = classicText.length > TRIM_LENGTH 
      ? classicText.substring(0, TRIM_LENGTH) 
      : classicText;
    ...
    console.log(`Original size: ${trimmedText.length} bytes`);
    ...
    if (compressionResult.compressionRatio < 1) {
      ...
      // Original format doesn't have dictionaries or references
      console.log('Dictionary and reference entries skipped in original format');
    } else {
    ...
    const decompressionResult = decompressString(
      ...
      'pride-and-prejudice.txt.utcp'
    );
    
    // The decompressed content should match the original
    // We normalize whitespace for the comparison by removing leading/trailing whitespace
    expect(decompressionResult.originalContent.trim()).toEqual(trimmedText.trim());
    
    // Also check content length to make sure we're not losing data
    const originalLength = trimmedText.trim().length;
    const decompressedLength = decompressionResult.originalContent.trim().length;
    console.log(`Original length: ${originalLength}, Decompressed length: ${decompressedLength}`);
    expect(Math.abs(decompressedLength - originalLength)).toBeLessThan(5); // Allow small differences
  });
});